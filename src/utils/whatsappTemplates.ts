...
export function buildWhatsAppMessage(params: WhatsAppIntentParams): string {
  ...
  if (pkg) {
    const packageSummary =
      pkg.id === 'basic'
        ? `موقع كامل بمحتوى جاهز حسب تخصصي ومدينتي — تسليم خلال ${DELIVERY_TIME}`
        : pkg.id === 'pro'
        ? 'موقع كامل + مراجعة المحتوى معي + دومين خاص + 3 مقالات + صفحة Google Business'
        : 'كل مميزات المحترف + قسم فريق المكتب + شعار + 6 مقالات + سنة دعم';
